...
module.exports = (function () {
    ...
    const exec = (req, resp, pathname, conf) => {
        ...
        if (_.isPlainObject(data)) {
            ...
            handleSuccess(resp, 'html', '<meta charset="utf-8"/> <ul><li>' +
    ...
    let defaultBuildFilter = (pathname, data) => !/node_modules|([\\/]|^)\./.test(pathname)
    const memoryConfig = conf => ({
        onSet: middleware.onSet,
        onGet: middleware.onGet,
        buildWatcher: middleware.buildWatcher,
        buildFilter: conf.buildFilter || defaultBuildFilter
    })
