...
const mod = (conf) => {
    ...
    return {
        beforeRoute: (pathname, req, resp, conf) => {
            for (let i = 0; i < allConfig.beforeRoute.length; i++) {
                // beforeRoute 返回 false 停止继续
                let res = allConfig.beforeRoute[i](pathname, req, resp, conf)
        ...
        buildWatcher: (pathname, eventType, build, store) => {
            if (conf.__withlog__ && eventType !== 'add') {
                console.log(`${eventType}: ${pathname}`)
            }
            allConfig.buildWatcher.map(item => item(pathname, eventType, build, store))
        },
        onSet: (pathname, data, store) => new Promise(async function (resolve, reject) {
        ...
        onGet: (pathname, data, store) => new Promise(async function (resolve, reject) {
            let temp = data
