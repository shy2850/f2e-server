...
module.exports = (conf) => {
    ...
    return {
        onSet (pathname, data, store) {
            ...
            switch (extType) {
            case 'js':
                let js = uglifyJs.minify(data.toString())
                store._set(pathname.replace(/\.(\w+)$/, '.js'), js.code)
                sourceMap && store._set(pathname.replace(/\.(\w+)$/, '.js.map'), js.map)
                break
            case 'css':
                let css = cleanCss.minify(data.toString())
                store._set(pathname.replace(/\.(\w+)$/, '.css'), css.styles)
                sourceMap && store._set(pathname.replace(/\.(\w+)$/, '.css.map'), css.sourceMap)
                break
            default:
            }
            return data
